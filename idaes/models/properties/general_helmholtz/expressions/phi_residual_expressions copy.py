...
def phi_residual_expressions_non_analytic(model, parameters):
    ...
    return {
        ...
        "phir_d": sum(
            ...
            * (
                bi[i] * ((model.delta - 1) ** 2) ** a[i]
                + (ai[i] * ((model.delta - 1) ** 2) ** (1 / (2 * beta)) - model.tau + 1)
                ** 2
            )
            ** b[i]
            * (
                ...
                * model.delta
                * (
                    a[i] * beta[i] * bi[i] * ((model.delta - 1) ** 2) ** a[i]
                    + ai[i]
                    * ((model.delta - 1) ** 2) ** (1 / (2 * beta))
                    * (
                        ai[i] * ((model.delta - 1) ** 2) ** (1 / (2 * beta))
                        - model.tau
                        + 1
                    )
                )
                ...
                - 2 * ci[i] * (model.delta - 1) * model.delta
                + 1
            )
            for i in rng
        ),
